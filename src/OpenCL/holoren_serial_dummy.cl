...
__kernel void compObjWave(__global float *pc,    /// a point cloud with point sources
                          ...
                          uint of_row_offset,    /// offset in row of the resulting optical field
                          uint of_col_offset,    /// offset in column of the resulting optical field
                          ...
                          float corner_y)        /// y corner of optical field
{
  ...
  float4 dist = (float4) (
    ((of_col_offset - 1) * pitch) + corner_x,
    ((of_row_offset - 1) * pitch) + corner_y,
    hologram_z,
    0.0f
  );
  ...
  //of[get_global_id(0) * get_global_size(1) + get_global_id(1)] = sample;
}
